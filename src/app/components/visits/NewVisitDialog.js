...
const mockClients = [
  { id: 1, name: "Comptoir Hammami",region: 'Ariana', distance: 300 },
  { id: 2, name: "COMAF", region: 'Sousse', distance: 800 },
];
...
function NewVisitDialog({ open, onClose, onSubmit }) {
  ...
  const handleNewClientSubmit = (newClientData) => {
    const clientWithDistance = {
      ...newClientData,
      distance: Math.random() * 1.5 * 1000,
    };

    setClients((prev) => [...prev, clientWithDistance]);
    setFormData((prev) => ({
      ...prev,
      clientId: clientWithDistance.name,
    }));
  ...
  const handleTerminer = () => {
    const selectedClient = clients.find((c) => c.name === formData.clientId);
    ...
    onSubmit({
      ...
      distance: selectedClient?.distance || 0,
      products: formattedProducts,
      observations: formData.observations,
      reclamations: formData.reclamations,
      duration,
      region: selectedClient?.region || "Sousse",
    });
  ...
  const clientOptions = [
    { title: "+ Nouveau client", value: "+ Nouveau client" },
    ...clients.map((client) => ({
      title: `${client.name} (${client.distance} m)`,
      value: client.name,
      disabled: client.distance > 500,
    })),
  ];
